    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('loans.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="borrower" id="borrower" class="w-full px-4 py-2 border rounded-lg @error('borrower') border-red-500 @enderror" value="{{ old('borrower') }}" required>
                @error('borrower')
            ...
            <div class="mb-4">
                ...
                <div class="flex items-center space-x-4">
                    <label class="inline-flex items-center">
                        <input type="radio" name="status" value="given" class="form-radio" {{ old('status', 'given') == 'given' ? 'checked' : '' }} required>
                        <span class="ml-2">Given</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="status" value="repaid" class="form-radio" {{ old('status') == 'repaid' ? 'checked' : '' }}>
