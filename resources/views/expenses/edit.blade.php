    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('expenses.update', $expense->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="amount" id="amount" class="w-full px-4 py-2 border rounded-lg @error('amount') border-red-500 @enderror" value="{{ old('amount', $expense->amount) }}" required>
                @error('amount')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date" id="date" class="w-full px-4 py-2 border rounded-lg @error('date') border-red-500 @enderror" value="{{ old('date', $expense->date->format('Y-m-d')) }}" required>
                @error('date')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="channel" id="channel" class="w-full px-4 py-2 border rounded-lg @error('channel') border-red-500 @enderror" required>
                    ...
                    @foreach($channels as $value => $label)
                        <option value="{{ $value }}" {{ (old('channel', $expense->channel) == $value) ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @error('channel')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="account_id" id="account_id" class="w-full px-4 py-2 border rounded-lg @error('account_id') border-red-500 @enderror" required>
                    ...
                    @foreach($accounts as $id => $name)
                        <option value="{{ $id }}" {{ (old('account_id', $expense->account_id) == $id) ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
                @error('account_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <textarea name="notes" id="notes" rows="3" class="w-full px-4 py-2 border rounded-lg">{{ old('notes', $expense->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="external_system_id" id="external_system_id" class="w-full px-4 py-2 border rounded-lg @error('external_system_id') border-red-500 @enderror">
                    ...
                    @foreach($systems as $id => $name)
                        <option value="{{ $id }}" {{ (old('external_system_id', $expense->external_system_id) == $id) ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
                @error('external_system_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-gray-500 text-xs mt-1">Select an external system to sync this expense record. Changing the system will trigger a new webhook.</p>
            </div>
