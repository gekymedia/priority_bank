...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 max-w-2xl">
        <form action="{{ route('admin.users.store') }}" method="POST">
            ...
            <div class="mb-4">
                <label for="account_id" class="block text-sm font-medium text-gray-700 mb-2">Account Id</label>
                <input type="text" name="account_id" id="account_id" value="{{ old('account_id') }}" 
                       placeholder="External system account identifier for debit/credit"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('account_id') border-red-500 @enderror">
                @error('account_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">Optional. Used by linked systems for debiting and crediting.</p>
            </div>
