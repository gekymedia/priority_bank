...
<body class="font-sans antialiased {{ $isDark ? 'bg-gray-900 text-gray-200' : 'bg-gray-50 text-gray-900' }}">
    <div class="flex h-screen">
        ...
        <div class="hidden md:flex md:flex-shrink-0">
                ...
                <div class="flex flex-col flex-grow px-4 py-4 overflow-y-auto">
                    <nav class="flex-1 space-y-2">
                        ...
                        @endif
                        @if(!auth()->user()->isAdmin())
                            <a href="{{ route('savings.index') }}" class="flex items-center px-4 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('savings.*') ? 'bg-indigo-600' : 'hover:bg-indigo-600' }}">
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
            ...
            <div id="mobile-sidebar" class="md:hidden hidden fixed inset-0 z-50 bg-black bg-opacity-50">
                <div class="fixed left-0 top-0 h-screen w-64 bg-indigo-700 text-white flex flex-col">
                    <!-- Sidebar Header -->
                    <div class="flex items-center justify-between px-4 py-4 border-b border-indigo-600">
                        <span class="text-xl font-bold">{{ config('app.name', 'PriorityBank') }}</span>
                        <button id="mobile-menu-close-header" class="text-white hover:text-indigo-200">
                    ...
                    @endif
                    @if(!auth()->user()->isAdmin())
                        <a href="{{ route('savings.index') }}" class="block px-4 py-2 text-base font-medium {{ request()->routeIs('savings.*') ? 'bg-indigo-600' : 'hover:bg-indigo-600' }}">
                            <i class="fas fa-piggy-bank mr-3"></i>
                            Savings
                        </a>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const mobileMenuClose = document.getElementById('mobile-menu-close-header');
