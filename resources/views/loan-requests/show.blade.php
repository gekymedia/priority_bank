...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <!-- Record Payment Button (Admin only, approved requests with loan) -->
        @if(Auth::user()->isAdmin() && $loanRequest->status === 'approved' && $loanRequest->loan)
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <form action="{{ route('loan-requests.record-payment', $loanRequest) }}" method="POST" onsubmit="return confirm('Record this loan payment as a transaction? This will create an expense transaction against Priority Bank source.');">
                @csrf
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                    <i class="fas fa-check-circle mr-2"></i> Record Transaction as Paid
                </button>
            </form>
        </div>
        @endif
            ...
            <form action="{{ route('loan-requests.approve', $loanRequest) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="amount_approved" name="amount_approved" 
                           value="{{ old('amount_approved', $loanRequest->amount_requested) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('amount_approved') border-red-500 @enderror"
                           step="0.01" min="1" max="{{ $loanRequest->amount_requested }}" required>
                    <p class="text-sm text-gray-500 mt-1">Maximum: GHS {{ number_format($loanRequest->amount_requested, 2) }}</p>
